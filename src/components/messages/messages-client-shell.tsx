...
import { searchRoomsAndUsersAction, createRoomAction } from "@/actions/messages-client";
...
export default function MessagesClientShell({ initialRooms, isAdmin, currentUserId }: MessagesClientShellProps) {
  ...
  const fetchMessages = async (roomId: string) => {
    try {
      const response = await fetch(`/api/messages/rooms/${roomId}/messages?limit=50`, {
        cache: "no-store",
      });
  ...
  const handleSearch = async (formData: FormData) => {
    ...
    // Note: In a real app, you'd want to update the rooms list here
  };
            ...
            if (payload.eventType === "INSERT") {
              ...
              setMessagesCache(prev => {
                ...
                if (existing.some(m => m.id === newMessage.id)) {
                  console.log(`Message ${newMessage.id} already exists, skipping duplicate`);
                  return prev;
                }
                ...
                console.log(`Added new message ${newMessage.id} to room ${room.id}`);
                return {
                  ...prev,
                  [room.id]: updated,
                };
          ...
          <div className="flex items-center gap-2">
            <form action={handleSearch} className="flex gap-2 flex-1">
              <input
                ...
                className="flex-1 border border-primary/20 rounded-full px-3 py-2 text-sm"
              />
            </form>
            {isAdmin && (
              <CreateRoomModal
                ...
                trigger={
                  <button className="cursor-pointer w-10 h-10 grid place-items-center border border-primary/20 rounded-full">
                    <Image src="/icons/add.svg" alt="plus" width={550} height={550} className="w-5 h-5" />
                  </button>
                }
              />
            )}
  ...
  return (
    <div className="h-[calc(100vh-74px)]">
      ...
      <div className="flex items-center gap-2 border-b border-primary/10 px-4 py-3 md:hidden">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium truncate">{selectedRoom?.name || "Conversations"}</p>
        </div>
      </div>
      ...
      <div className="hidden md:block h-full">
        <ResizablePanelGroup direction="horizontal" className="h-full">
          <ResizablePanel defaultSize={30} minSize={20} maxSize={40} className="h-full">
            <aside className="h-full py-6 border-r border-primary/10 overflow-y-auto">
              {RoomsList}
      </aside>
          </ResizablePanel>
          <ResizableHandle withHandle />
          <ResizablePanel className="h-full">
            <section className="h-full overflow-y-auto pt-4">
        ...
        <section className="h-full overflow-y-auto">
          {!selectedRoomId || !selectedRoom ? (
            <div className="h-full w-full flex items-center justify-center p-6 text-center">
              <p className="text-foreground/60">Open the menu to choose a conversation</p>
            </div>
          ) : (
            <ChatRoom
              key={selectedRoomId}
